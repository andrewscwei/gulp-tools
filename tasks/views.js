...
function build(config, locale, done) {
  ...
  if (locale && config.i18n && (config.i18n.locales instanceof Array) && (locale === config.i18n.locales[0])) locale = undefined;
...
function normalizeConfig(config, locale) {
  ...
  if (config.tags) {
    if ((typeof config.tags.layout === 'string') && _.isEmpty(path.extname(config.tags.layout))) config.tags.layout = `${config.tags.layout}.${config.layouts.engine}`;
  ...
  if (config.collections) {
    for (let collectionName in config.collections) {
      ...
      if (paginate) {
        if ((typeof paginate.layout === 'string') && _.isEmpty(path.extname(paginate.layout))) paginate.layout = `${paginate.layout}.${config.layouts.engine}`;
